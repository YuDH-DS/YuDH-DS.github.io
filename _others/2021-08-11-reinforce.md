...
정책이란 Agent가 각 상태에서 취해야할 행동에 대한 집합이다. 강화 학습은 랜덤으로 초기 정책을 설정한 후 Agent가 환경과 상호작용하는 과정 속에서 정책을 수정해 나가며 보상의 총합을 극대화하는 정책을 찾는 것을 목표로한다. (b)에서 Agent가 행동을 수정하며 찾아낸 정책은 아래와 같다.
<br>
<br>
$\phi_{(b)} = [East, North, East, North, West, West, West]
