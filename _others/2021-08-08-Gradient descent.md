...
### 1. Definition ###
 ...
 대다수의 ML/DL은 다양한 모델에 훈련용 데이터를 대입해 target의 수치 또는 분류를 예측한다. 그리고 검증치와 예측치 사이에 발생한 손실을 최소화하는 방안을 찾아가며 스스로 학습을 수행한다. 따라서 손실함수에 대한 최소화 메커니즘은 ML/DL 모델의 성능을 결정짓는 가장 중요한 요소이며, 그 최소화 메커니즘이 바로 경사하강법(Gradient descent)이다. ML/DL 모델은 지정된 초기값에서 시작해 점차 경사의 일정 비율만큼 값을 감소시키면서 최소값을 탐색해 나가며 구체적인 메커니즘은 아래의 수식과 같다.
 
 (a = Learning rate = 라그랑지안 승수, f(x) = Loss function = 손실함수)
