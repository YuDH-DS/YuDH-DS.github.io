...
SQL은 DBMS이다. 따라서 SQL에서 분석을 진행할 경우 번거로운 데이터 추출 작업 없이 분석을 진행할 수 있다는 장점이 있다. 그러나 빅데이터 시대로 진입하며 데이터 분석은 보다 직관적인 시각화를 필요로하며 SQL은 이 점에서 다소 부족한 점이 사실이다. 따라서 최근 다양한 데이터 분석이 Tableau와 같은 시각화 툴을 통해 진행되고 있다. [tabelau를 활용한 cohort 분석의 예시][2]를 이곳에서 확인해볼 수 있다. 
